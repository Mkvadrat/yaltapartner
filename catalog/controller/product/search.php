...
class ControllerProductSearch extends Controller { 	
	public function index() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_tag'])) {
            ...
            $product_total = $this->model_catalog_product->getTotalSearchProducts($data);
								
			//$results = $this->model_catalog_product->getProducts($data);
            $results = $this->model_catalog_product->getSearchProducts($data);
...
//                $rub = $this->currency->format($this->tax->calculate($rub, $result['tax_class_id'], $this->config->get('config_tax')));
        ...
        if (count($this->data['products']) == 1 && !empty($this->data['products'][0]['href'])) {
            $this->response->redirect(html_entity_decode($this->data['products'][0]['href']));
        } else {
