...
class ModelModuleFilterPro extends Model {
	...
	private function normalizeVersion() {
		return (int)str_pad(str_replace(".", "", VERSION), 4, "0");
	}
		...
		if($data['attribute_value']) {
			if($this->filterpro_setting['attribute_mode'] == 'and') {
				...
				foreach($data['attribute_value'] as $attribute_id => $values) {
					if($this->filterpro_setting['attribute_value_mode'] == 'or') {
...
//			$sql .= " AND realprice >=" . (int)$min_price;
            ...
            foreach ($currencies as $currency) {
                if ($data['valute'] == 'rub') {
                    $price_sql[] = "(realprice >=" . (int)($this->currency->convert((int)$data['min_price'], 'RUB', $currency['code'])) . " AND currency_id = '".((int)$currency['currency_id'])."')";
                } else {
                    $price_sql[] = "(realprice >=" . (int)($this->currency->convert((int)$data['min_price'], 'USD', $currency['code'])) . " AND currency_id = '".((int)$currency['currency_id'])."')";
                }
            }
...
//			$sql .= " AND realprice <=" . ($max_price);
            ...
            foreach ($currencies as $currency) {
                if ($data['valute'] == 'rub') {
                    $price_sql[] = "(realprice <=" . (int)($this->currency->convert((int)$data['max_price'], 'RUB', $currency['code'])) . " AND currency_id = '".((int)$currency['currency_id'])."')";
                } else {
                    $price_sql[] = "(realprice <=" . (int)($this->currency->convert((int)$data['max_price'], 'USD', $currency['code'])) . " AND currency_id = '".((int)$currency['currency_id'])."')";
                }
            }
            $sql .= " AND (".implode(" OR ", $price_sql).")";
		}
